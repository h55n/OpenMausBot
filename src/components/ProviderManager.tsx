...
/** Creates a bounded slug used as the base of managed API instance ids. */
function slug(value: string): string {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 40) || "provider";
}

/** Returns whether an instance belongs to the managed API-provider lifecycle. */
...
/** Returns whether the selected provider preset requires a user-supplied base URL. */
...
/** Renders and manages API-backed provider connections alongside existing account-based instances. */
export function ProviderManager() {
  ...
  /** Reloads the canonical instance list from the server. */
  ...
  /** Refreshes one managed connection's model catalog while ignoring stale concurrent responses. */
  ...
  /** Resets transient form state without altering saved provider connections. */
  const resetForm = () => {
    setOpen(false);
    setApiKey("");
    setBaseUrl("");
  ...
  /** Validates and persists a new managed API-provider connection. */
  ...
  /** Removes a confirmed managed API-provider connection through the narrow config mutation. */
