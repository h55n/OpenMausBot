...
/** Returns the immutable catalog of API provider connection presets. */
export function providerConnectionPresets(): readonly ProviderConnectionPreset[] {
  return PRESETS;
}

/** Resolves a provider preset by id and rejects unknown ids. */
...
/** Trims provider base URLs and removes trailing slashes for stable request construction. */
function normalizeBaseUrl(value: string): string {
  return value.trim().replace(/\/+$/, "");
}

/** Validates provider endpoints and permits insecure HTTP only for loopback development hosts. */
...
/** Rejects empty provider secrets and control characters that are unsafe in environment storage. */
...
/** Produces a bounded URL-safe slug for generated managed provider ids. */
...
/** Allocates a collision-safe managed provider instance id from a display name. */
...
/** Normalizes validated provider form input into a managed OpenAI-compatible instance configuration. */
...
/** Identifies instances created through the managed API-provider lifecycle. */
